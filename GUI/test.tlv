...
\TLV
   ...
   *failed = 1'b0;
   ...
   //  │    ├─ Signal scanner        — reads wd.signals              │
   ...
   /viz
      \viz_js
         ...
         init() {
            ...
            // Hotkeys are registered once here in init() — they persist.
            // ----------------------------------------------------------
            self.Events = {
               zones: [],
            ...
            // Mouse drag handling + coordinate transform.
            ...
            // EXTENSION POINT — add more bridges here:
            //   session.on("cycle-update", ...)
            //   session.on("vcd", ...)
            //   VI.ide.patch("my_param", value)
         },
         ...
         //
         // UPGRADE NOTES:
         //   • Signal scanner:   change the filter string to target other pipelines
         //   • Semantic sorter:  expand the regex to recognise more signal patterns
         ...
         onTraceData() {
            ...
            // ── Signal scanner ──────────────────────────────────────
            // Change "TLV|chip$" to target a different pipeline.
            const rawSigs = Object.keys(wd.signals)
                              .filter(k => k.startsWith("TLV|chip$"));

            // ── Semantic sorter ─────────────────────────────────────
            // Add new patterns to the regex to classify more signals.
            // Currently: anything containing out, res, tx, data → right side (output).
            const outputs = [];
            const inputs  = [];
            rawSigs.forEach(path => {
               const name  = wd.signals[path].notFullName.replace("$", "");
               const isOut = /out|res|tx|data/i.test(name);
               (isOut ? outputs : inputs).push({ name: name.toUpperCase(), path });
            });

            // ── Layout engine ───────────────────────────────────────
            // pinSpacing controls vertical gap between pins.
            // Chip height grows automatically based on pin count.
            const pinSpacing = 60;
            ...
            // ── Manifest writer ─────────────────────────────────────
            // Add new fields to this object to pass data to render().
            this.Manifest = {
               chipX:    860,
               chipY:    100,
               width:    chipWidth,
            ...
            M.pins.push({ id: "GND", name: "GND", dx: 0, dy: chipHeight - 40, side: "L", path: null, type: "gnd" });

            // Right-side outputs
            // Auto VCC (top-right)
            M.pins.push({ id: "VCC", name: "VCC", dx: chipWidth, dy: 40, side: "R", path: null, type: "vcc" });
            outputs.forEach((p, i) => {
               M.pins.push({
                  id:   "R" + i,
                  name: p.name.substring(0, 6),
            ...
            // EXTENSION POINT — add new chip config overrides here:
            // self.ChipConfigs = { NE555: { ... }, LVC1G04: { ... } };
         },
         ...
         render() {
            ...
            // ── LAYOUT CONSTANTS ──────────────────────────────────────
            // Adjust SCOPE to resize the oscilloscope panel.
            ...
            // ── L1 — ROUTER ───────────────────────────────────────────
            // 3-tier spatial routing:
            //   Tier 1 (scope wrap):  probe is behind the oscilloscope → wrap left
            //   Tier 2 (chip wrap):   probe is behind the chip body    → wrap right
            //   Tier 3 (free-space):  standard Manhattan
            //
            // UPGRADE: replace pts calculation with A* or force-directed layout.
            // UPGRADE: group probes on same pin into a bus bundle.
            S.probes.forEach((probe, idx) => {
               ...
               // Drop shadow (z-bottom, behind everything)
            ...
            // ── L2 — CHIP ─────────────────────────────────────────────
            // Renders from Manifest data — fully data-driven.
            // Click chip body to toggle x-ray mode.
            //
            // UPGRADE: add multi-chip support by iterating a chips[] array.
            // UPGRADE: add bus lines between chips.
            // UPGRADE: draw internal schematic in x-ray mode from graph pane.
            E.add(M.chipX, M.chipY, M.width, M.height, () => {
               S.showXRay = !S.showXRay;
               self.redraw();
            });

            // Render pins before package body (body covers them partially — correct)
            M.pins.forEach(p => {
            ...
            if (!S.showXRay) {
               ...
               // EXTENSION POINT: draw internal gates here using x-ray mode.
               // Example pattern from the original engine:
               //   D.poly(cvs, "x_gate", [[M.chipX+20,cy-10],[M.chipX+20,cy+10],[M.chipX+40,cy],[M.chipX+20,cy-10]], "#aaa", 2, false);
            }
            ...
            // ── L3 — SCOPE ────────────────────────────────────────────
            // Oscilloscope panel with grid, waveform traces, cursor line.
            //
            // UPGRADE: add persistence mode (ghost traces from prev cycles).
            // UPGRADE: add FFT mode (use Math to compute frequency spectrum).
            // UPGRADE: add trigger mode (hold display until edge condition).
            D.rect(cvs, "sc_bg",
            ...
            // ── L4 — PROBE HEADS ──────────────────────────────────────
            // Always rendered last (top z-order).
            // Wire joins hidden underneath probe body — center-tap design.
            //
            // UPGRADE: add color picker on long-press.
            // UPGRADE: add trigger mode toggle per probe.
            ...
            // EXTENSION POINT — add L5, L6... blocks here following the
            // same pattern. Each block reads M and S, writes to cvs via D.
            // Example L5 ideas:
            //   • Logic analyser (hex decode of multi-bit buses)
            //   • Timing ruler (measure pulse width between two probes)
            //   • Signal table (text readout of all pin values at CYC)
            //   • Bus decoder (I2C / SPI pattern matching)
         }
\SV
   endmodule
